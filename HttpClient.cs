...
namespace WeChatAPI
{
    ...
    public class WxHttpClient
    {
        ...
        {
            FileLog.Log($"GetString Requset {url}");
            var result = Retry<string>(() =>
            {
        ...
        {
            FileLog.Log($"GetStringOnce Requset {url}");
            ...
            using (stream)
            {
                ...
                result = sr.ReadToEnd();
                FileLog.Log($"GetStringOnce Response {result}");
            }
            return result;
        }
        ...
        {
            FileLog.Log($"PostFormString Requset {url}");
            string result = string.Empty;
            Stream stream = Retry<Stream>(() =>
            {
            ...
            using (stream)
            {
                StreamReader sr = new StreamReader(stream);
                result = sr.ReadToEnd();
                FileLog.Log($"PostFormString Response {result}");
            }
            return result;
        }
        ...
        {
            FileLog.Log($"PostJson Requset {url}");
            string result = string.Empty;
            Stream stream = Retry<Stream>(() =>
            {
            ...
            using (stream)
            {
                StreamReader sr = new StreamReader(stream);
                result = sr.ReadToEnd();
                FileLog.Log($"PostJson Response {result}");
            }
